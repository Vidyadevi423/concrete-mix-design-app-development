...
if (!empty($in['new_password'])) {
    ...
    // If current_password is provided, verify it. Otherwise, allow password update for authenticated users.
    // (UI has been simplified to not ask for current password.)
    $cur = $in['current_password'] ?? null;
    if ($cur !== null && $cur !== '') {
