...
function initNavToggle() {
  ...
  sidebar.querySelectorAll('a').forEach(a => {
    a.addEventListener('click', () => {
      // On mobile, close the drawer after selecting a page. On desktop, keep it open.
      if (mobileMq.matches) setSidebarState(false);
    });
  });

  document.addEventListener('click', function (e) {
    // Only auto-close when it's a mobile drawer.
...
async function syncUserFromServer() {
  // If a server session / remember-cookie exists, keep localStorage in sync
  // so the UI shows the correct user info across pages.
  try {
    const r = await fetch('get_user.php', { credentials: 'same-origin' });
    const data = await r.json().catch(() => null);
    if (data && data.success && data.data) {
      localStorage.setItem('concrete_logged_in', '1');
      localStorage.setItem('concrete_user_name', data.data.full_name || data.data.name || 'User');
      localStorage.setItem('concrete_user_email', data.data.email || '');
      localStorage.setItem('concrete_user_role', data.data.role || 'Engineer');
    }
  } catch (_) {}
}
